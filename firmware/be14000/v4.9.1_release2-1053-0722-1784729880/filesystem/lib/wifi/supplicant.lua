...
function supp_setup_vif(dev, iface)
	local file_name = var_supplicant_path.."wpa_supplicant-"..iface.ifname..".conf"
	local file
	...
	if iface.ignore_old_scan_res ~= nil then
		io.write("ignore_old_scan_res=", iface.ignore_old_scan_res, "\n")
	else
		if dev.map_mode ~= nil and dev.map_mode ~= "0" then
			io.write("ignore_old_scan_res=1\n")
		end
	end
	...
	if iface.default_disabled == "1" then
		io.write("\tdisabled=1\n")
	end
...
--[[
	...
	if iface.encryption and string.find(iface.encryption, "wpa") ~= nil then
	...
	if iface.encryption and string.find(iface.encryption, "dpp") ~= nil then
...
function supp_enable_vif(iface)
	local file_name = var_supplicant_path.."wpa_supplicant-"..iface.ifname..".conf"
	local action_scan_pid = var_path.."/action-"..iface.ifname.."-scan.pid"

	if iface.network or iface.multi_ap == "1" then
	--	os.execute("ubus -t 20 wait_for network.interface."..iface.network)
	--	os.execute(wpa_cli.." -p "..var_supplicant_path.." -i global interface_add "..iface.ifname.." "..file_name.." \"\" \"\" \"\" br-"..iface.network)
	--else
		os.execute(wpa_cli.." -p "..var_supplicant_path.." -i global interface_add "..iface.ifname.." "..file_name.." \"\" \"\" \"\" \"\"")
	end
	os.execute("exec 1000>&- && "..wpa_cli.." -i "..iface.ifname.." -a /lib/wifi/supplicant_scan_action.sh -B -P "..action_scan_pid)
end
