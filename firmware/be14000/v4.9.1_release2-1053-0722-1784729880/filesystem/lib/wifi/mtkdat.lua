...
local EHT_BW_160 = 3
local EHT_BW_320 = 4
local gl_country = io.popen("cat /proc/gl-hw-info/country_code"):read("*l")
if gl_country == nil or gl_country == "" then
  gl_country = "US"
end
local l1dat_parser = {
...
local country_region_map_2g = {
    CA = 0, CO = 0, DO = 0, GT = 0, MX = 0, NO = 0, PA = 0, PR = 0, TW = 0, US = 0, UZ = 0
}

local country_region_map_5g = {
    AL = 0, DZ = 0, AU = 0, BH = 0, BY = 0, CL = 0, CN = 0, CO = 0, CR = 0, DO = 0, EC = 0, SV = 0, GT = 0, HN = 0, HK = 0, IN = 0, IL = 0, JO = 0, KZ = 0, KW = 0,
    LB = 0, MO = 0, MK = 0, MY = 0, MX = 0, MA = 0, NZ = 0, NO = 0, OM = 0, PK = 0, PA = 0, PR = 0, QA = 0, RO = 0, RU = 0, SA = 0, SG = 0, SY = 0, TH = 0, UA = 0, AE = 0,
    VN = 0, YE = 0, ZW = 0,
    AT = 1, BE = 1, BR = 1, BG = 1, CY = 1, DK = 1, EE = 1, FI = 1, DE = 7, GR = 1, HU = 1, IS = 1, IE = 1, IT = 1, LV = 1, LI = 1, LT = 1, LU = 1, NL = 1, PL = 1, PT = 1,
    SK = 1, SI = 1, ZA = 1, ES = 1, SE = 1, CH = 1, GB = 1, UZ = 1,
    AM = 2, AZ = 2, HR = 2, CZ = 2, EG = 2, FR = 2, GE = 2, MC = 2, TT = 2, TN = 2, TR = 2,
    AR = 3, TW = 3, BZ = 4, BO = 4, BN = 4, ID = 4, IR = 4, PE = 4, PH = 4,
    KP = 5, KR = 5, UY = 5, VE = 5, JP = 12, CA = 13, US = 13
}

local country_region_map_6g = {
    AU = 1, BR = 0, CA = 0, CL = 0, CO = 0, DO = 0, GB = 0, GT = 0, HK = 1, JO = 0, JP = 1, KP = 0, KR = 0, KW = 0, LB = 0, MX = 0, NO = 0, NZ = 0, OM = 0, PA = 0, PE = 0,
    PH = 0, PR = 0, QA = 0, SA = 0, TW = 0, US = 0, UY = 0, DE = 1
}
...
function mtkdat.check_band_profile(path)
    local ret = false
    ...
    if content and string.find(content, "BN0_profile_path", 1, true) then
        ret = true
    end

    fd:close()
    if ret == false then
        local src_path = "/rom"..path
        nixio.syslog("info", "mtwifi: "..path.." not found, copy from "..src_path)
        os.execute(string.format("cp -f %s %s", src_path, path))
    end
end

function mtkdat.check_dat_file(path)
    local ret = false
    if type(path) ~= "string" or not path:match("%.dat$") then
        return
    end
    ...
    for i = 1, 3 do
        ...
        if line:match("^Default$")  ~= nil  then
            ret = true
            break
        end
    end
...
local uci_dev_options = {
    ...
    "dfs_slave", "cp_support", "band4_dfs_enable", "dfs_zero_wait_bandidx", "random_bssid"
}
...
local uci_iface_options = {
    ...
    "macaddr", "proxy_arp", "ht_mpdu_density", "ifname"
}
...
local function cfg2dev(cfg, devname, dev)
    ...
    dev.random_bssid = cfg.RandomBSSID
...
--        dev.bw = "20"
    ...
    if dev.band == "5G" or dev.band == "5g" then
        dev.acs_skiplist = "52;56;60;64;100;104;108;112;116;120;124;128;132;136;140;144;149;153;157;161;165"
    elseif dev.band == "6G" or dev.band == "6g" then
        if gl_country == "US" then
            dev.acs_skiplist = "97;101;105;109;113;117;121;125;129;133;137;141;145;149;153;157;161;165;169;173;177;181;185;189;193;197;201;205;209;213;217;221;225;229;233"
        else
            dev.acs_skiplist = "73;77;81;89;93;97;105;109;113;121;125;129;137;141;145;153;157;161;169;173;177;185;189;193;197;201;205;209;213;217;221;225;229;233"
        end
    elseif dev.band == "2.4G" or dev.band == "2g" then
        dev.acs_skiplist = "12;13"
    else
...
local function cfg2iface(cfg, devname, ifname, iface, i)
    ...
    if auth_list[i] == nil then
        auth_list[i] = "OPEN"
    end
    if encr_list[i] == nil then
        encr_list[i] = "NONE"
    end
...
--[[
    ...
    iface.eht_t2lmnegosupport = cfg.EHT_ApcliT2lmNegoSupport
...
local function htmode2bw(band, htmode, pure_11b)
    ...
    if string.upper(htmode) == 'NOHT' then
        ...
        elseif string.upper(band) == "5G" or
               string.upper(band) == "5g" then
    ...
    elseif string.upper(htmode) == 'HE20' then
        ...
        elseif string.upper(band) == "6G" or
               string.upper(band) == "6g" then
...
local function dev2cfg(dev, cfg, devname)
    ...
    if dev.band == "5G" or dev.band == "5g" then
        cfg.AutoChannelSkipList = dev.acs_skiplist or "52;56;60;64;100;104;108;112;116;120;124;128;132;136;140;144;149;153;157;161;165"
    elseif dev.band == "2.4G" or dev.band == "2g" then
        cfg.AutoChannelSkipList = dev.acs_skiplist or "12;13"
    elseif dev.band == "6G" or dev.band == "6g" then
        if gl_country == "US" then
            cfg.AutoChannelSkipList = dev.acs_skiplist or "97;101;105;109;113;117;121;125;129;133;137;141;145;149;153;157;161;165;169;173;177;181;185;189;193;197;201;205;209;213;217;221;225;229;233"
        else
            cfg.AutoChannelSkipList = dev.acs_skiplist or "73;77;81;89;93;97;105;109;113;121;125;129;137;141;145;153;157;161;169;173;177;185;189;193;197;201;205;209;213;217;221;225;229;233"
        end
    else
    ...
    cfg.RandomBSSID = dev.random_bssid or "0"
    ...
    if dev.band == "2g" then
        cfg.CountryRegion = country_region_map_2g[cfg.CountryCode] or "1"
    elseif dev.band == "5g" then
        cfg.CountryRegionABand = country_region_map_5g[cfg.CountryCode] or "13"
    elseif dev.band == "6g" then
        cfg.CountryRegionABand = country_region_map_6g[cfg.CountryCode] or "1"
    end
...
--    cfg.WirelessMode = token_set(cfg.WirelessMode, i, iface.wireless_mode)
    ...
    elseif iface.eml_mode == "disable" or iface.eml_mode == nil then
...
local function clear_apcli_cfg(cfg)
    if cfg.ApCliEnable == "0" then
        return
    end
    cfg.ApCliEnable = "0"
    cfg.ApCliSsid = ""
    cfg.ApCliBssid = ""
    cfg.ApcliMacAddress = ""
    cfg.ApCliAuthMode = "OPEN"
    cfg.ApCliEncrypType = "NONE"
    cfg.ApCliWPAPSK = ""
    cfg.ApCliPMFMFPC = '0'
    cfg.ApCliPMFMFPR = '0'
    cfg.ApCliPMFSHA256 = ''
    ...
    for j = 1, 4 do
        ...
        cfg["ApCliKey"..tostring(j).."Str"] = ''
    end
    cfg.ApCliPweMethod = "0"
    cfg.ApCliWirelessMode = ""
...
--[[
    ...
    elseif auth == "WPA2" or auth == "WPA3" then
        cfg.ApCliWPAPSK = iface.password
    end

    cfg.ApcliEapType = iface.eap_type or "0"
    cfg.ApcliIdentity = iface.identity or ""
    ...
    cfg.EHT_ApcliT2lmNegoSupport = iface.eht_t2lmnegosupport
...
function mtkdat.dat2uci()
    ...
    for devname, profile in mtkdat.spairs(profiles, function(a,b) return string.upper(a) < string.upper(b) end) do
        ...
        j = 1
        while i <= if_num do
            local ssid_key = "SSID"..tostring(i)
            local ssid_val = cfg[ssid_key]
             -- 检查SSID非空才配置
            if ssid_val and ssid_val ~= "" then
                uci["wifi-iface"][m] = {}
                iface = uci["wifi-iface"][m]
                ifname = main_ifname..(j-1)
                cfg2iface(cfg, dev[".name"], ifname, iface, j)
                -- 仅当配置成功时递增
                m = m + 1
                j = j + 1
                nixio.syslog("debug", "mtwifi, uci ifname:"..ifname.." devname:"..dev[".name"].." m:"..m.." j:"..j)
            end

            i = i + 1
        end

        --When ApCliEnable is 1, configure the apcli interface
        if cfg.ApCliEnable == "1" then
...
function mtkdat.uci2dat(uci)
    if not mtkdat.exist(uciCfgfile) then return end

    if not uci then
        uci = mtkdat.uci_load_wireless()
    end
    local profiles = mtkdat.search_dev_and_profile()
    local l1dat, l1 = mtkdat.__get_l1dat()
    local cfgs = {}
    ...
    for _, dev in pairs(uci["wifi-device"]) do
        ...
        mtkdat.check_dat_file(profiles[devname])
        cfg = mtkdat.load_profile(profiles[devname])
        old_cfg = table_clone(cfg);
        for id = 1, 63 do
