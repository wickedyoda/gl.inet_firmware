...
load_rule()
{
    load_rule_cb(){
        local config=$1
        if [ "$config" = "drop" ] || [ "$config" = "accept" ]; then
            json_init
        ...
        local file_path="/tmp/$1.conf"
        [ "$manual" != "0" ] && [ -s "$file_path" ] && {
            local json_content
            json_content=$(cat "$file_path")
        }
        config_apply "$json_content"
    }
