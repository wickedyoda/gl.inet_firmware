...
local function connect(self, options)
    ...
    local res = ubus.call('gl-session', 'resolve', { host = request_host }) or {}
    local address = res.address

    if not address then
        return nil, "failed to resolve host: " .. request_host
    end
    ...
    elseif not request_port then
        ...
        ok, err = sock:connect(address[1], tcp_opts)
        if not ok then
            return nil, err
        end
        ...
        ok, err = sock:connect(address[1], request_port, tcp_opts)
        if not ok then
            return nil, err
        end
    end
