...
function _M.recv_frame(sock, max_payload_len, force_masking)
    ...
    if opcode == 0x8 then
        ...
        if payload_len > 0 then
            ...
            local msg, code
            if mask then
                fst = bxor(byte(data, 4 + 1), byte(data, 1))
                snd = bxor(byte(data, 4 + 2), byte(data, 2))
                code = bor(lshift(fst, 8), snd)

                if payload_len > 2 then
                    msg = {}
                    for i = 3, payload_len do
                        msg[i - 2] = char(bxor(byte(data, 4 + i),
                                            byte(data, (i - 1) % 4 + 1)))
                    end
                    msg = concat(msg)
                else
...
local function build_frame(fin, opcode, payload_len, payload, masking)
    ...
    if masking then
        ...
        local masked = {}
        for i = 1, payload_len do
            masked[i] = char(bxor(byte(payload, i),
