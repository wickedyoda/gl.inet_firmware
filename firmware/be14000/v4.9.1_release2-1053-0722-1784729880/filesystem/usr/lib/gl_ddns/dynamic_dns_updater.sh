...
#
# variables in small chars are read from /etc/config/gl_ddns
...
case "$1" in
	...
	stop)
		if [ -f "/usr/lib/gl_ddns/get_ddns_post_cmd.lua" ]; then
			__cmd=$(lua /usr/lib/gl_ddns/get_ddns_post_cmd.lua clean)
			eval $__cmd
		fi
		if [ -n "$INTERFACE" ]; then
...
load_all_config_options "gl_ddns" "$SECTION_ID"
...
[ $ERR_LAST -ne 0 ] && {
	...
	write_log  7 "uci configuration:${N}$(uci -q show gl_ddns | grep '=service' | sort)"
...
write_log 5 "PID '$$' started at $(eval $DATE_PROG)"
write_log 7 "ddns version  : $VERSION"
write_log 7 "uci configuration:${N}$(uci -q show gl_ddns.$SECTION_ID | sort)"
...
[ -n "$service_name" ] && {
	...
	get_service_data "$service_name" "/usr/share/gl_ddns/custom" update_url update_script UPD_ANSWER
	if [ "$?" != "0" ]; then
		get_service_data "$service_name" "/usr/share/gl_ddns/default" update_url update_script UPD_ANSWER
	fi
}
...
[ -z "$lookup_host" ] && {
	uci -q set gl_ddns.$SECTION_ID.lookup_host="$domain"
	uci -q commit gl_ddns
	lookup_host="$domain"
}
...
while : ; do
	...
	get_registered_ip REGISTERED_IP	"NO_RETRY" # get registered/public IP
