...
if [ -f /sbin/l1dat ]; then
	...
	card1_main=`l1dat zone2if dev00 | awk '{print $1}'`
	card1_ext=`l1dat zone2if dev00 | awk '{print $2}'`
	card2_main=`l1dat zone2if dev01 | awk '{print $1}'`
	card2_ext=`l1dat zone2if dev01 | awk '{print $2}'`
	card3_main=`l1dat zone2if dev02 | awk '{print $1}'`
	card3_ext=`l1dat zone2if dev02 | awk '{print $2}'`
...
card1_exist=`ifconfig | grep ${card1_main}`
card2_exist=`ifconfig | grep ${card2_main}`
card3_exist=`ifconfig | grep ${card3_main}`
...
if [ "$all_inf_list" != "" ]; then
	...
	# Background process wapp
	wapp -d1 -v2 $wapp_intf_name -iNL80211 &
fi
./etc/init.d/wapp stop
